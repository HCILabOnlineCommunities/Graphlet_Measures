...
# progressive rating
def progressiveness_wi_commun(k, snap_shots, comm_graph, graphlet_type, commun, graphlet_lp):
    ...
    for graphlet_count, graphlet in enumerate(graphlets):
        ...
        graphlet_lp[graphlet_type][graphlet_count] = {} #subg.edges
        # if type(graphlet) != type(())  tuple
        # graphlet = tuple(graphlet)
        ...
        graphlet_lp[graphlet_type][graphlet_count]["LP_AVG"]= sum_LP/len(graphlet_lp[graphlet_type][graphlet_count]["LP"])
        LP_sum = LP_sum + graphlet_lp[graphlet_type][graphlet_count]["LP_AVG"]    
    
    if len(graphlets) == 0:
        return NO_GRAPH_DATA
    
    return LP_sum/len(graphlets)



if __name__ == "__main__":
    # Expect first argument to sys.argv is a community file name
    # print("Community:", sys.argv[1])
    # commun = sys.argv[1]
    LP_AVG_graphlet = {}
    ...
    for commun in top_100_commun:
        ...
        for g in graphlet_type:
            graphlet_lp[g] = {}
            LP = progressiveness_wi_commun(12, snap_shots, comm_graph, g, commun, graphlet_lp)
            if g not in LP_AVG_graphlet:
                LP_AVG_graphlet[g] = [0,0,0]
            
            if LP != NO_GRAPH_DATA:
                LP_AVG_graphlet[g][0] += 1 
                LP_AVG_graphlet[g][1] += LP
